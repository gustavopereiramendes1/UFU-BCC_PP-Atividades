...
int main(){
    ...
    do{
        qnt = 0;
        printf("\nDigite o mes e o ano (ex. 10/2003): ");
        scanf("%d/%d" , &m, &a);

        printf("\nCompromissos encontrados:\n\n");
        for(int i = 0; i < NUM_COMPROMISSO; i++){
            if(agenda[i].data.mes == m && agenda[i].data.ano == a){
                printf("Compromisso %d: %s " , qnt + 1, agenda[i].compromisso);
                printf("Data: %d/%d/%d" , agenda[i].data.dia, agenda[i].data.mes, agenda[i].data.ano);
                printf("\n\n");
                qnt++;
            }
